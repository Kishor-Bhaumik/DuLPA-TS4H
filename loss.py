...
class Bidirectional_aligner(nn.Module):
    ...
    def forward(self, mu_s: torch.Tensor, f_t: torch.Tensor, bbse_weights: torch.Tensor) -> torch.Tensor:
        prop = bbse_weights.view(-1, 1).to(mu_s.device)

        sim_mat = torch.matmul(mu_s, f_t.T)
        new_logits = self.get_pos_logits(sim_mat, prop)
        ...
        target_loss = (((1-self.s_par) * cost_mat * t_dist).sum(1) * prop.squeeze(1)).sum()
